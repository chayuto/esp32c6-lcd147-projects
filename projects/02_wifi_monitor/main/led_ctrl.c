...
static void fade_to(uint8_t tr, uint8_t tg, uint8_t tb, uint32_t ms)
{
    ...
    for (int i = 1; i <= steps; i++) {
        uint8_t r = (uint8_t)(s_cur_r + (int)(tr - s_cur_r) * i / steps);
        uint8_t g = (uint8_t)(s_cur_g + (int)(tg - s_cur_g) * i / steps);
        uint8_t b = (uint8_t)(s_cur_b + (int)(tb - s_cur_b) * i / steps);
        set_raw(r, g, b);
...
void led_ctrl_set_cu(double cu_pct)
{
    if (!s_enabled) return;
    if (cu_pct < 40.0) {
        led_ctrl_set(0, 51, 10, 500);   // dim green
    } else if (cu_pct < 70.0) {
        led_ctrl_set(60, 25, 0, 500);   // dim orange
    } else {
        led_ctrl_set(60, 0, 0, 500);    // dim red
    }
}

void led_ctrl_boot_breathing(void)
{
    // Non-blocking: queue a single dim-white fade-in
    led_ctrl_set(30, 30, 30, 800);
}
